...
| Category | % of reference Report | Achievable Without LLM? |
|----------|---------------------|------------------------|
...
**Goal:** Produce reference-equivalent output for all computable sections, as a CLI tool, at $0/run.
...
│   ├── output/
│   │   ├── markdown.ts           # reference-format markdown report
...
Validation is NOT "eyeball it against reference reports." It is a structured, repeatable process:
...
**Cost estimate:** 4 focused prompts × ~2K tokens input × ~1K tokens output = ~12K total tokens per analysis = **~$0.05/run** (vs reference's ~$3/run).
...
|-------------------|-------------|----------|--------|
| React Dashboard | 10-15K | CUT | No buyer needs it. reference itself outputs markdown. |
...
2. **Validate demand** — Contact 5 potential buyers (email reference, Mechanize, Poolside). Share sample reports. Do they want this?
3. **Validate the reference submission angle** — If the primary goal is creating a codebase for reference, does this tool + its codebase meet their requirements? (50K+ LOC, test coverage, commit history)
...
|------|------------|--------|------------|
| Output is "just SonarQube but worse" | High | Fatal | Differentiate on: reference-specific metrics (test discipline, commit-test co-occurrence), scoring rubric optimized for AI training data evaluation, markdown-first output |
| No buyers at any price point | Medium | High | Validate in weeks 5-6 before investing more. Pivot to using the tool for own reference submissions. |
