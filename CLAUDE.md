...
A TypeScript CLI tool that produces Proximal-equivalent codebase analysis reports without LLM dependency. Orchestrates existing tools (scc, jscpd, gitleaks) + tree-sitter AST parsing to compute static metrics, then outputs scored markdown/JSON reports. Solo developer: Aditya.
...
## Reference Reports
Three Proximal reports are the format target (all in `/Users/aditya/projects/Random/Proximal/`):
