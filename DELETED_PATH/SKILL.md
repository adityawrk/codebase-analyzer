...
description: Compare analyzer output against a reference report for the same repo. Shows section-by-section coverage and value gaps. One-shot.
user_invocable: true
---

# Match reference

Compare the codebase analyzer's output against a reference reference report for the same repository. Identifies section-by-section coverage, metric accuracy, and formatting differences.

## When to Invoke

- When the user runs `/match-reference <reference-report-path>` with the path to a reference report
- Optionally: `/match-reference <reference-report-path> <analyzer-output-path>` to compare specific files
- During validation to measure how close our output is to reference quality

## Agent Instructions

### Step 1: Load Reports

1. Read the reference reference report (markdown file provided by user)
...
### Step 2: Parse reference Report Sections

Break the reference report into its known sections:
...
For each reference section, report:
...
1. **Coverage:** Does our analyzer produce equivalent data?
   ...
   - `BETTER` — we produce more detail than reference

2. **Accuracy (for static metrics):** Compare our values to reference's
...
| reference Section | Coverage | Accuracy | Format | Notes |
|-----------------|----------|----------|--------|-------|
| Summary | FULL | Exact | Match | |
| Language Breakdown | FULL | LOC off by 2% | Table format differs | scc vs reference counting |
...
| Metric | reference Value | Our Value | Match? |
|--------|---------------|-----------|--------|
